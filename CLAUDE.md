# Ashiato Claude Code Guide

このリポジトリでは `cc-sdd` を採用し、Claude Code では `CLAUDE.md` と `.kiro/steering/` を長期メモリとして扱います。作業開始時は必ずリポジトリルートを基準にし、`frontend/` `api/` `bot/` `doc/` を横断して影響範囲を確認してください。
